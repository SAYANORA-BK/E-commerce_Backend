...
namespace E_commerce.Service
{
    public class CartService : ICartService
    {
        ...
        public async Task<ApiResponse<CartItems>> AddToCart(int productId, int userid)
        {
            try
            {
                ...
                if (isuser == null)
                {
                    return new ApiResponse<CartItems>(404, "User not found");
                }

                var isproduct = await _context.Product.FirstOrDefaultAsync(x => x.ProductId == productId);
                if (isproduct == null)
                {
                    return new ApiResponse<CartItems>(404, "Product not found");
                }
                ...
                {
                    return new ApiResponse<CartItems>(404, "Out of stock");
                }
                ...
                await _context.SaveChangesAsync();
                return new ApiResponse<CartItems>(200, "Successfully added to cart");
            }
            catch (Exception ex)
            {
                return new ApiResponse<CartItems>(500, ex.Message);
            }
        }
